...
长时间无人值守运行使用监督模式。每轮都会完整回收浏览器和 OCR 资源；异常、页面暂停或单轮超时后按配置重启，并通过单实例锁避免重复运行。运行器还会用浏览器原生输入处理播放器受保护提示；媒体在低就绪态连续 10 秒没有进度时自动刷新并从站点断点恢复；页面主线程连续 10 秒不响应状态读取时整轮重启，避免导航到考试页后浏览器渲染进程卡死。考试题库在运行中补全后会提交一次新的全量已知答案组合，不会被旧的组合计数提前截断。`status.json` 使用原子替换保留完整最新状态；`events.ndjson` 追加精简事件并默认按 10 MiB 轮转、保留 3 个历史文件，避免长期运行无限占用磁盘：
...
另外覆盖异步页面加载、真实 `cid`/`cwid`、问卷恢复弹窗/隐藏选项/滑块/排序题、互动病例、考试异常验证码、播放器受保护提示的浏览器原生输入、低就绪态停滞恢复、现代培训卡组合自动选择、无卡课程跳过继续其余任务并轮询全部待申请课程、重复注入清理、课程目录去重与跨页面恢复。Hermes 默认每 5 分钟启动一轮培训卡复查（可用 `--card-retry-minutes` 调整），Tampermonkey 默认每 30 分钟复查。Hermes 使用受长度约束的跨子域状态桥，仅同步流程游标和阻塞项，完整学习记录与任务队列继续保留在各页面本地存储中。
