(function () {
  ...
  function handleStudy(retry) {
    ...
    var task = plan.tasks.find(function (item) { return item.type === 'apply' && item.record.url; });
    if (task) {
      setState({
    ...
    var localApply = info.localTasks.find(function (item) {
      return item.record.status === 'completed' && /申请证书|申请学分|确认/.test(actionText(item.element));
    });
    if (localApply) {
      setState({ phase: 'certificate', message: '申请学分：' + localApply.record.name });
  ...
  function handleCard() {
    ...
    if (/可用培训卡\s*\(?0\)?|这里空空的|暂无.*培训卡/.test(text)) {
      setState({ running: false, paused: true, phase: 'card', message: '没有可用培训卡' });
      return;
    }
